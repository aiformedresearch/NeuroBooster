      ...
      for backbone in "${backbones[@]}"; do
        for labels_percentage in "${labels_percentages[@]}"; do
          ...
          if [[ ( "$labels_percentage" -eq 100 || "$paradigm" == "supervised" ) && ! -f "$pretrain_DONE_FILE" ]]; then
            ...
            CUDA_VISIBLE_DEVICES=2 python source/pretraining_deit_LARS_3D_single_gpu_oldexp.py \
              --paradigm ${paradigm} \
          ...
          if [[ ! -f "$finetune_DONE_FILE" ]]; then
            ...
            CUDA_VISIBLE_DEVICES=2 python source/fine_tune_evaluate_deit_3D_single_gpu_oldexp.py \
              --paradigm ${paradigm} \
              --images_dir ${images_dir} \
              --tabular_dir ${tabular_dir} \
              --dataset_name ${dataset_name} \
