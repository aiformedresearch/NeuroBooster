...
pretrain_base_lr=${pretrain_base_lr:-0.0000001} #0.0000001
    ...
    for dataset_name in "${datasets[@]}"; do
      for backbone in "${backbones[@]}"; do
        for labels_percentage in "${labels_percentages[@]}"; do
          EXPERIMENT_FOLDER_NAME=../REVISION1/EXPERIMENTS_ABLATION_2025_07_08_3D_actually_noaugm_resnet34_long_distributed_batch32/seed${seed}/${dataset_name}/${paradigm}/labels_percentage_${labels_percentage}
          ...
          if [[ ( "$labels_percentage" -eq 100 || "$paradigm" == "supervised" ) && ! -f "$pretrain_DONE_FILE" ]]; then
            ...
            TORCH_DISTRIBUTED_DEBUG=DETAIL torchrun --nproc_per_node=4 --master_port=29502 source/pretraining_deit_LARS_3D.py \
              --paradigm ${paradigm} \
          ...
          #   torchrun --nproc_per_node=4 --master_port=29502 source/fine_tune_evaluate_deit_3D.py \
          #     --paradigm ${paradigm} \
          #     --images_dir ${images_dir} \
          #     --tabular_dir ${tabular_dir} \
          #     --dataset_name ${dataset_name} \
...
for seed in "${seeds[@]}"; do
  for paradigm in "${paradigms[@]}"; do
    for dataset_name in "${datasets[@]}"; do
      for backbone in "${backbones[@]}"; do
        for labels_percentage in "${labels_percentages[@]}"; do
          EXPERIMENT_FOLDER_NAME=../REVISION1/EXPERIMENTS_ABLATION_2025_07_08_3D_actually_noaugm_resnet34_long_distributed_batch8/seed${seed}/${dataset_name}/${paradigm}/labels_percentage_${labels_percentage}
          FOLD0_FOLDER="${EXPERIMENT_FOLDER_NAME}/fold_0"
          pretrain_DONE_FILE="${FOLD0_FOLDER}/pretraining_done.txt"
          finetune_DONE_FILE="${FOLD0_FOLDER}/finetuning_ablation_done.txt"
          mkdir -p "$FOLD0_FOLDER"
