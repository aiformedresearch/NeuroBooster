...
import utils.dataset_utils_new_paradigm as dataset_utils_new_paradigm
...
def main(args):
    ...
    if args.cross_val_folds == 1:
        # bootstrapping
        indexes_train_folds, _, targets_train_folds, _, _, tabular_scaler_folds, args.num_classes  = dataset_utils_new_paradigm.bootstrap(args)
    else: 
        # cross validation
        indexes_train_folds, _, targets_train_folds, _, _, tabular_scaler_folds, args.num_classes  = dataset_utils_new_paradigm.cross_validation(args)
    ...
    for fold in range(args.cross_val_folds):
        ...
        if ('beit' in args.backbone) and (args.paradigm != 'simim'):
            print(f'{args.backbone} is being changed to "deit" which is the only ViT tested architecture for paradigms that are not simim')
            args.backbone='deit'
        ...
        train_dataset = dataset_utils_new_paradigm.ADNI_AGE_Dataset(args, targets_train_fold_i, indexes_train_fold_i, transforms)
