...
from models.NeuroBooster import init_neurobooster, init_neurobooster_deit
...
from losses.NeuroBooster_loss import neurobooster_loss
...
def get_arguments():
    ...
    parser.add_argument("--paradigm", type=str, default="supervised", help='choices: supervised neurobooster vicreg simim mae')
...
def main(args):
    ...
    if not((args.paradigm == 'supervised') or (args.paradigm == 'neurobooster')) and ('3D' in args.backbone):
        raise Exception('3D implemented only for supervised and neurobooster paradigms')
    ...
    indexes_train, _, targets_train, _, _, tabular_scaler, args.num_classes  = dataset_utils.bootstrap(args)

    last_state = {}

    args.exp_dir = args.exp_dir_original
    ...
    if args.paradigm == 'vicreg':
    ...
    elif (args.paradigm == 'supervised') or (args.paradigm == 'neurobooster'):
        if '3D' in args.backbone:
            ...
            transforms = [data_augmentations.neurobooster_and_supervised_augmentations.TrainTransform_Crop_Affine_Noise, # augmentation
                        data_augmentations.neurobooster_and_supervised_augmentations.TrainTransform_Crop, # default transform
                        ]

    elif args.paradigm == 'neurobooster_corrupted':
        transforms = [data_augmentations.neurobooster_and_supervised_augmentations.TrainTransform_Crop_Affine_Noise, # augmentation
                        data_augmentations.neurobooster_and_supervised_augmentations.TrainTransform_Crop, # default transform
                        data_augmentations.neurobooster_and_supervised_augmentations.corrupt]
    ...
    if '3D' in args.backbone:
        train_dataset = dataset_utils.ADNI_AGE_Dataset_3D(args, targets_train, indexes_train, transforms)

    else:
        train_dataset = dataset_utils.ADNI_AGE_Dataset(args, targets_train, indexes_train, transforms)

    ################## MODEL ARCHITECTURE:
    print('INITIALIZING MODEL')
    if args.paradigm in ['neurobooster', 'supervised', 'neurobooster_corrupted']:
    ...
    ########## LOSS FUNCTION
    if args.paradigm == 'supervised' or args.paradigm == 'neurobooster' or args.paradigm == 'neurobooster_corrupted':

        if 'deit' in args.backbone:
            print('init deit backbone')
            model = init_neurobooster_deit(args).cuda(gpu)
            print(model)
        else:
            if '_3D' in args.backbone:
                ...
                model = init_neurobooster(args).cuda(gpu)

        model.head = nn.Sequential(model.head)
        
        if args.task == 'regression':
            print('regression loss')
            criterion = neurobooster_loss().cuda(gpu)
        elif args.task == 'classification':
            print('classification loss')
            sampled_classes = [0,1]
            if args.weighted_loss and args.paradigm == 'supervised':
                class_0_count = len(targets_train[targets_train==0])
                class_1_count = len(targets_train[targets_train==1])
    ...
    if args.optim == 'LARS':
        ...
        if not(('beit' in args.backbone) or ('deit' in args.backbone)):
            raise ValueError('AdamW_short strategy implemented only for ViT')
        else:
            lr = args.base_lr
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, (img_x, img_y, tabular, original_img, samples_id) in enumerate(loader, start = epoch * len(loader)):   
            ...
            with torch.cuda.amp.autocast():
                if args.paradigm == 'mae':
                ...
                elif (args.paradigm == 'neurobooster') or (args.paradigm == 'neurobooster_corrupted') or (args.paradigm == 'supervised') :
                    ...
                    with torch.no_grad():
                        if args.task == 'regression':
                            if tabular_scaler:
                                rescaled_loss = criterion(torch.tensor(tabular_scaler.inverse_transform(output.cpu())).cuda(gpu,non_blocking=True), torch.tensor(tabular_scaler.inverse_transform(tabular.cpu())).cuda(gpu,non_blocking=True))
                            else:
                                rescaled_loss = loss
...
def create_dict_state(args, model, optimizer, epoch):        
    ...
    elif (args.paradigm in ['supervised','neurobooster','neurobooster_corrupted']) and ('resnet' in args.backbone):
        state = dict(
        epoch=epoch + 1,
