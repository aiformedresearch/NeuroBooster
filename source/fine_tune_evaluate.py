...
from models.NeuroBooster import init_neurobooster, init_neurobooster_deit, init_neurobooster_3D
...
def get_arguments():
    ...
    parser.add_argument("--use_augmentations", type = str2bool, default =True)
...
def main_worker(gpu, args):
    ...
    indexes_train, indexes_val, targets_train, targets_val, _, tabular_scaler, args.num_classes  = dataset_utils.bootstrap(args)
    ...
    print(f'TRAINING number of samples after considering labels percentage: {len(targets_train)}')
    print(f'VALIDATION number of samples after considering labels percentage: {len(targets_val)}')
    ...
    if '3D' in args.backbone:
        ...
        train_dataset = dataset_utils.ADNI_AGE_Dataset_3D(args, targets_train, indexes_train, train_transform)
        val_dataset = dataset_utils.ADNI_AGE_Dataset_3D(args, targets_val, indexes_val, val_transform, train_mean=train_dataset.mean, train_std=train_dataset.std)

    else:
        train_transform = [TrainTransform_Resize] 
        val_transform = [ValTransform_Resize]
        train_dataset = dataset_utils.ADNI_AGE_Dataset(args, targets_train, indexes_train, train_transform)
        val_dataset = dataset_utils.ADNI_AGE_Dataset(args, targets_val, indexes_val, val_transform, train_mean=train_dataset.mean, train_std=train_dataset.std)
    ...
    if 'deit' in args.backbone:
        ...
        if args.pre_training_paradigm == 'mae':
            ...
            # Use init_neurobooster_deit initialization
            print("Paradigm is NOT 'mae': using init_neurobooster_deit for fine-tuning")
        ...
        # Note: head keys are different if using init_neurobooster_deit
        ...
        raise Exception(f'{args.backbone} is not among the possible backbones')
        ...
        for step, (img_x, img_y, tabular, original_img, samples_id) in enumerate(train_loader, start=epoch * len(train_loader)):
            ...
            with torch.cuda.amp.autocast():  # automaitc mixed precision
                if 'deit' in args.backbone:
                    ...
                    if args.freeze_backbone:
                        with torch.no_grad():
                            if epoch == 0 and step == 0: print('backbone freezed')
                            backbone.requires_grad_(False)
                ...
                with torch.no_grad():
                        ...
                        if args.task == 'regression':
                            if tabular_scaler:
                                finetuning_rescaled_loss = criterion(torch.tensor(tabular_scaler.inverse_transform(output.cpu())), torch.tensor(tabular_scaler.inverse_transform(tabular.cpu())))
                            else:
            ...
            for step, (img_x, img_y, tabular, original_img, samples_id)in enumerate(val_loader, start=epoch * len(val_loader)):
                with torch.cuda.amp.autocast():  # automaitc mixed precision
                    ...
                    with torch.no_grad():
                        if args.task =='regression':
                            if tabular_scaler:
                                val_rescaled_loss = criterion(torch.tensor(tabular_scaler.inverse_transform(output.cpu())), torch.tensor(tabular_scaler.inverse_transform(tabular.cpu())))
                                val_MAE_loss = criterion_MAE(torch.tensor(tabular_scaler.inverse_transform(output.cpu())), torch.tensor(tabular_scaler.inverse_transform(tabular.cpu())))
                            else:
