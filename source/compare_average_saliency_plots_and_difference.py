...
def plot_multiple_overlays(paths, save_path=None):
    ...
    # Compute differences with SL
    if sl_saliency is not None:
        for i in range(len(paradigms)):
            if i == sl_index:
                continue
            diff = np.mean(np.abs(saliencies[i] - sl_saliency))
            paradigms[i] += f" ({diff:.3f})"
...
if __name__ == '__main__':
    ...
    output_file = "saliency_comparison_with_difference.png"
...
# Example usage
resize_image("saliency_comparison_with_difference.png", "saliency_comparison_with_difference_resized.png")
