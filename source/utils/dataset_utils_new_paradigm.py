...
def build_subject_index_dict(groups, train_indexes):
    """
    Build a dictionary mapping each subject to a list of its image indexes in the training set.
    
    Args:
        groups (list): A list mapping index to subject ID (e.g., ['patient1', 'patient2', ...]).
        train_indexes (list): A list of indexes (integers) belonging to the training set.
        
    Returns:
        dict: {subject_id: [list of indexes in train_indexes]}
    """
    subject_index_dict = {}
    
    for idx in train_indexes:
        subject_id = groups[idx]
        if subject_id not in subject_index_dict:
            subject_index_dict[subject_id] = []
        subject_index_dict[subject_id].append(idx)
...
def analyze_subject_image_distribution(subjects_dict):
    """
    Analyze the distribution of images per patient and print:
    - total number of patients
    - average and standard deviation of images per patient
    - count of patients having each possible number of images (0 to max)
    
    Args:
        subjects_dict (dict): {subject_id: [list of indexes]}
        
    Returns:
        dict: summary of statistics
    """
    # Get counts of images per patient
    image_counts = [len(indexes) for indexes in subjects_dict.values()]
    
    # Calculate average and std
    average_images = np.mean(image_counts)
    std_images = np.std(image_counts)
    
    # Determine maximum number of images
    max_images = max(image_counts)
    
    # Count how many patients have 0, 1, 2, ..., max_images images
    # (Note: in practice, patients with 0 images won't be in the dict)
    counter = Counter(image_counts)
    
    histogram_counts = {i: counter.get(i, 0) for i in range(0, max_images + 1)}
    
    # Print results
    print("Total number of patients:", len(subjects_dict))
    print("Average number of images per patient:", average_images)
    print("Standard deviation:", std_images)
    print("Histogram of patients by number of images:")
    for i, count in histogram_counts.items():
        print(f"  {i} images: {count} patients")
    
    # Return stats in a dict for further use
    stats = {
        'total_patients': len(subjects_dict),
...
def bootstrap(args):
    ...
    if 'AGE' in str(args.dataset_name):
        ...
        # indexes for patient-aware contrastive learning:
        train_subjects_dict = build_subject_index_dict(groups, indexes_train)
        stats = analyze_subject_image_distribution(train_subjects_dict)
        print(stats)
        er
