...
for row_idx, labels_percentage in enumerate(labels_percentage_list):
    for col_idx, metric in enumerate(metrics):
        ...
        paradigms_names = {'supervised': 'SL', 'neurobooster': 'NB', 'vicreg': 'VICReg', 'simim': 'SimMIM', 'baseline': 'baseline', 'mae': 'MaskedAE', 'simclr': 'SimCLR'}
        ...
        if dataset_name == 'ADNI':
            ...
            xmin, xmax = manual_x_limits[metric][labels_percentage]
            axs[row_idx, col_idx].set_xlim(xmin, xmax)
        else:
            axs[row_idx, col_idx].set_xlim(x_min - 1, x_max + 1)

        # ✅ Y-axis fix: boost KDE height only for ADNI
        if dataset_name == 'ADNI':
            from scipy.stats import gaussian_kde
            y_max = 0
            for paradigm in df_results.columns:
                values = df_results[paradigm].dropna()
                if len(values) > 1:
                    kde = gaussian_kde(values, bw_method=bw_adjust)
                    x_vals = np.linspace(values.min() - 1, values.max() + 1, 1000)
                    kde_vals = kde(x_vals)
                    y_max = max(y_max, kde_vals.max(), y_max)
            axs[row_idx, col_idx].set_ylim(0, y_max * 1.5)
        else:
...
legend_handles = [
    Line2D([0], [0], color=paradigm_colors[p], linestyle=paradigm_linestyles[p], linewidth=4, label=legend_labels[p])
    for p in legend_labels
]
...
plt.savefig(destination_folder_path / f'new_center_title_combined_plots_definitive_{metrics}_{bw_adjust}_{cut}_{dpi}dpi.png', dpi=dpi, bbox_inches='tight')
plt.close()
