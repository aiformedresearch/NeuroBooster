...
def create_raw_comparison_data(experiment_folders: list, dataset_name: str = 'AGE', metrics: list = ['MAE'],
                            paradigms: list = ['supervised', 'medbooster', 'vicreg', 'simim', 'mae'],
                            seeds_range: range = range(5), folds_range: range = range(1),
                            labels_percentage_list: list = [100, 1]) -> pd.DataFrame:
    """
    Collects raw mean and standard deviation for specified metrics from experiment folders.
    """

    all_results = []
    paradigms_names = {'supervised': 'SL', 'medbooster': 'MB', 'vicreg': 'VICReg', 'simim': 'SimMIM', 'baseline': 'baseline', 'mae': 'MAE'}

    seeds_str_in_filename = str(seeds_range)
    folds_str_in_filename = str(folds_range)
    paradigms_str_in_filename = str(paradigms)

    for experiment_folder_root in experiment_folders:
        experiment_name = experiment_folder_root.name
        ...
        for labels_percentage in labels_percentage_list:
            for metric in metrics:
                results_file_name = (
                    ...
                    f'_seeds_{seeds_str_in_filename}'
                    f'_folds_{folds_str_in_filename}'
                    f'_labels_percentage_{labels_percentage}'
                    f'_paradigms_{paradigms_str_in_filename}.csv'
                )
                ...
                try:
                    ...
                    for paradigm_display_name in paradigms_names.values():
                        if paradigm_display_name in df.columns:
                            mean_val = df[paradigm_display_name].mean()
                            std_val = df[paradigm_display_name].std()

                            all_results.append({
                                'Experiment': experiment_name,
                                'Labels Percentage': labels_percentage,
                                'Metric': metric,
                                'Paradigm': paradigm_display_name,
                                'Mean': mean_val,
                                'Std': std_val
                            })
...
if __name__ == "__main__":
    ...
    experiment_folders_to_compare = [
        experiment_root_folder1,
        experiment_root_folder2,
        experiment_root_folder3
    ]

    # Parameters for data collection (adjust these if necessary to match your experiments)
    dataset_name = 'AGE'
    metrics = ['MAE']
    seeds = range(5)
    folds = range(1)
    paradigms = ['supervised', 'medbooster', 'vicreg', 'simim', 'mae']
    labels_percentage_list = [100, 1] # Ensure this list contains the percentages you expect

    # --- Data Collection ---
    raw_comparison_df = create_raw_comparison_data(
        experiment_folders=experiment_folders_to_compare,
    ...
    if raw_comparison_df.empty:
        ...
        for labels_percentage in labels_percentage_list:
            ...
            file_suffix = f"labels_{labels_percentage}percent"
