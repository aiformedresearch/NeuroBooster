    ...
    for dataset_name in "${datasets[@]}"; do
      for backbone in "${backbones[@]}"; do
        for labels_percentage in "${labels_percentages[@]}"; do
          EXPERIMENT_FOLDER_NAME=../REVISION1/EXPERIMENTS_ABLATION_2025_06_23_3D_actually_noaugm_resnet34_long_accum_iter_updated/seed${seed}/${dataset_name}/${paradigm}/labels_percentage_${labels_percentage}
          ...
          if [[ ! -f "$finetune_DONE_FILE" ]]; then
            ...
            CUDA_VISIBLE_DEVICES=0 python source/fine_tune_evaluate_deit_3D.py \
              --paradigm ${paradigm} \
              --images_dir ${images_dir} \
              --tabular_dir ${tabular_dir} \
              --dataset_name ${dataset_name} \
